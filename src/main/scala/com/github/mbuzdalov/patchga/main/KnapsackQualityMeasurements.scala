  ...
  private def algorithmList(name: String): Seq[(String, SupportedOptimizer)] =
    name match
      case "default" => Seq(
        ...
        "NFGA [dist 1.5 other 2.5]" -> new NeverForgettingGA(2.5, 1.5, 0.5, 1.5, 2.5, d => PowerLawDistribution(d - 1, 2.5).symmetric(d)),
        "NFGA [all 1.5]" -> new NeverForgettingGA(1.5, 1.5, 0.5, 1.5, 1.5, d => PowerLawDistribution(d - 1, 1.5).symmetric(d)),
      )
